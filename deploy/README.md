...
runRokaiXplorer()
```
Note that, after running the ```runRokaiXplorer()``` command, the application will stay open until it is closed. So, *please make sure to close the application before moving on the next steps and running the next code snippets*. 
